    ...
    for (const data of allData) {
      ...
      if (!result.success) {
        if (response.status === 409) {
          console.warn(`Duplicate ProjectPool detected: ${data.ProjectPool}`);
          showErrorToUser(`Cannot save: ProjectPool '${data.ProjectPool}' already exists`);
        } else {
      ...
      existingProjectPools.add(data.ProjectPool); // Only add if successful
    }
    return allSaved;
  } catch (error) {
